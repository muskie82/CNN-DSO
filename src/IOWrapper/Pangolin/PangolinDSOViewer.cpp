...
PangolinDSOViewer::PangolinDSOViewer(int w, int h, bool startRunThread)
{
...

	{
		...
		videoImgChanged=kfImgChanged=depthmapImgChanged=true;
...
void PangolinDSOViewer::run()
{
	...
	while( !pangolin::ShouldQuit() && running )
	{
		...
		videoImgChanged=kfImgChanged=depthmapImgChanged=false;
...
void PangolinDSOViewer::pushCNNImage(const cv::Mat& depth)
{
	...
	for(int y=0; y<h; y++){
		for(int x=0; x<w; x++){
			internalDepthMapImg->data[y*w+x][0] = depthmap_visual.at<Vec3b>(y,x)[0];
			internalDepthMapImg->data[y*w+x][1] = depthmap_visual.at<Vec3b>(y,x)[1];
			internalDepthMapImg->data[y*w+x][2] = depthmap_visual.at<Vec3b>(y,x)[2];
